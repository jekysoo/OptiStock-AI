...
# Smart path resolution for multi-environment deployment
...
# ИСПРАВЛЕНО: Теперь везде используется правильный метод metric()
col1.metric(label="🌟 Active SKU Count in System", value=f"{unique_skus:,} Items")
col2.metric(label="🛑 Frozen Capital (Dead Stock)", value=f"{frozen_capital / 1e9:,.2f} Bln UZS")
col3.metric(label="📈 Parsed 1C Transactions", value=f"{total_rows:,} Entries")
